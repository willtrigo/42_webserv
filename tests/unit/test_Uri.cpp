...
/*   Updated: 2026/01/06 19:28:15 by umeneses         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
TEST_F(UriTest, ConstructWithInvalidUri) {
  ...
  EXPECT_THROW(Uri("http://www.example.com:99999"),
               domain::http::exceptions::UriException);
}

TEST_F(UriTest, ParseMalformedUri_invalid_port_format) {
  EXPECT_THROW(Uri("http://example.com:abc"),
               domain::http::exceptions::UriException);
  EXPECT_THROW(Uri("http://example.com:-80"),
               domain::http::exceptions::UriException);
  EXPECT_THROW(Uri("http://example.com:0"),
               domain::http::exceptions::UriException);
  EXPECT_THROW(Uri("http://example.com:65536"),
               domain::http::exceptions::UriException);
}
...
TEST_F(UriTest, ParseMalformedUri_invalid_scheme) {
  EXPECT_THROW(Uri("123://example.com"),
               domain::http::exceptions::UriException);
  EXPECT_THROW(Uri("-http://example.com"),
               domain::http::exceptions::UriException);
}
...
TEST_F(UriTest, ParseMalformedUri_too_long_uri) {
  ...
  EXPECT_THROW(
      { Uri uri(tooLongUri); }, domain::http::exceptions::UriException);
}
...
TEST_F(UriTest, InvalidAuthorityParsing) {
  ...
  EXPECT_THROW(Uri::parseAuthority(":"),
               domain::http::exceptions::UriException);
  EXPECT_THROW(Uri::parseAuthority("www.example.com:99999"),
               domain::http::exceptions::UriException);
  EXPECT_THROW(Uri::parseAuthority("host:port:extra"),
               domain::http::exceptions::UriException);
}
...
TEST_F(UriTest, ParseUriWithQueryOnly) {
  ...
  EXPECT_EQ("", uri.getPath());  // RFC 3986: empty path, not "/"
...
TEST_F(UriTest, GetPort) {
  ...
  EXPECT_EQ(80, uri3.getPort().getValue());

  Uri uri4("/relative/path");
  EXPECT_EQ(80, uri4.getPort().getValue());
}
...
TEST_F(UriTest, GetPath) {
  ...
  Uri uri3("http://example.com");
  EXPECT_EQ("", uri3.getPath());  // RFC 3986: empty path, not "/"
...
TEST_F(UriTest, ValidateShouldRejectNullBytes) {
  ...
  uriWithNull += "/path";
  Uri* u = NULL;
  EXPECT_THROW(u = new Uri(uriWithNull),
               domain::http::exceptions::UriException);
  if (u) delete u;
}
...
TEST_F(UriTest, ParseSchemeShouldRejectSchemeStartingWithNumber) {
  EXPECT_THROW(Uri("1http://example.com"),
               domain::http::exceptions::UriException);
}

TEST_F(UriTest, ParseSchemeShouldRejectSchemeWithSpaces) {
  EXPECT_THROW(Uri("ht tp://example.com"),
               domain::http::exceptions::UriException);
}
...
TEST_F(UriTest, ParseAuthorityShouldRejectMalformedIPv4) {
  EXPECT_THROW(Uri("http://256.256.256.256/path"),
               domain::http::exceptions::UriException);
  EXPECT_THROW(Uri("http://192.168.1/path"),
               domain::http::exceptions::UriException);
}

TEST_F(UriTest, ParseAuthorityShouldRejectMalformedIPv6) {
  EXPECT_THROW(Uri("http://[::gggg]/path"),
               domain::http::exceptions::UriException);
  EXPECT_THROW(
      Uri("http://[::1/path"),
      domain::http::exceptions::UriException);  // Missing closing bracket
}
...
TEST_F(UriTest, ExtractPortShouldRejectPortZero) {
  EXPECT_THROW(Uri("http://example.com:0/path"),
               domain::http::exceptions::UriException);
}

TEST_F(UriTest, ExtractPortShouldRejectPortAboveMaximum) {
  EXPECT_THROW(Uri("http://example.com:65536/path"),
               domain::http::exceptions::UriException);
  EXPECT_THROW(Uri("http://example.com:99999/path"),
               domain::http::exceptions::UriException);
}

TEST_F(UriTest, ExtractPortShouldRejectNonNumericPort) {
  EXPECT_THROW(Uri("http://example.com:abc/path"),
               domain::http::exceptions::UriException);
}

TEST_F(UriTest, ExtractPortShouldRejectNegativePort) {
  EXPECT_THROW(Uri("http://example.com:-80/path"),
               domain::http::exceptions::UriException);
}

TEST_F(UriTest, ExtractPortShouldRejectEmptyPort) {
  EXPECT_THROW(Uri("http://example.com:/path"),
               domain::http::exceptions::UriException);
}
