...
/*   Updated: 2025/12/30 19:30:11 by umeneses         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
TEST_F(CgiConfigTest, ConstructWithRelativeScriptPath) {
  ...
  EXPECT_THROW(CgiConfig("relative/path", cgiRoot, pattern), CgiConfigException);
}
...
TEST_F(CgiConfigTest, SetCgiRootEmpty) {
  CgiConfig config;
  domain::filesystem::value_objects::Path emptyPath("");
  
  EXPECT_THROW(config.setCgiRoot(emptyPath), CgiConfigException);
}

TEST_F(CgiConfigTest, SetCgiRootRelative) {
  CgiConfig config;
  domain::filesystem::value_objects::Path relativePath("cgi-bin");
  
  EXPECT_THROW(config.setCgiRoot(relativePath), CgiConfigException);
}
...
TEST_F(CgiConfigTest, IsValidFalse) {
  domain::filesystem::value_objects::Path cgiRoot("/usr/lib/cgi-bin");
  domain::shared::value_objects::RegexPattern pattern("\\.php$");
  CgiConfig config("", cgiRoot, pattern);
...
TEST_F(CgiConfigTest, ValidateEmptyScriptPath) {
  CgiConfig config;
  config.addParameter(CgiConfig::DEFAULT_SCRIPT_FILENAME, "/path/to/script.php");
  config.addParameter(CgiConfig::DEFAULT_REQUEST_METHOD, "GET");
  config.addParameter(CgiConfig::DEFAULT_SERVER_PROTOCOL, "HTTP/1.1");
  config.addParameter(CgiConfig::DEFAULT_GATEWAY_INTERFACE, "CGI/1.1");
...
TEST_F(CgiConfigTest, ClearRestoresDefaults) {
  ...
  EXPECT_EQ("CGI/1.1", config.getParameter(CgiConfig::DEFAULT_GATEWAY_INTERFACE));
}
